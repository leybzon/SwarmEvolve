...
def plot_arms_race_timeline(team_a: list, team_b: list, out_path: Path):
    ...
    ax.plot(a_rounds, a_fitness, 'o-', color='#4A90E2', alpha=0.3,
            linewidth=1, markersize=3, label='Team A attempts')
    ax.plot(b_rounds, b_fitness, 'o-', color='#E24A4A', alpha=0.3,
            linewidth=1, markersize=3, label='Team B attempts')

    # Plot accepted champions (bold markers)
    ax.scatter(a_accepted, a_accepted_fit, s=100, marker='o',
               color='#4A90E2', edgecolor='black', linewidth=1.5,
               zorder=10, label='Team A accepted')
    ax.scatter(b_accepted, b_accepted_fit, s=100, marker='o',
               color='#E24A4A', edgecolor='black', linewidth=1.5,
               zorder=10, label='Team B accepted')
    ...
    if b_accepted and max(b_accepted_fit) > 0.5:
        ...
        ax.annotate(
            ...
            fontsize=10,
            bbox=dict(boxstyle='round,pad=0.5', facecolor='#FFE5E5', edgecolor='#E24A4A', linewidth=2),
    ...
    ax.set_title('M25 Co-Evolution: Competitive Arms Race\n100 Rounds, Alternating Team Evolution',
    ...
    ax.text(22.5, -1.05, 'Phase 2:\nTeam B Evolution', ha='center', fontsize=8, style='italic', alpha=0.7)
...
def plot_champion_staircase(team_a: list, team_b: list, out_path: Path):
    ...
    # Plot staircases
    ax.step(a_rounds, a_fit, where='post', linewidth=3, color='#4A90E2',
            label='Team A Champion', zorder=5)
    ax.step(b_rounds, b_fit, where='post', linewidth=3, color='#E24A4A',
            label='Team B Champion', zorder=5)

    # Mark transitions
    for r, f in a_accepted[1:]:  # Skip first
        ax.scatter([r], [f], s=150, marker='D', color='#4A90E2',
                   edgecolor='black', linewidth=1.5, zorder=10)
    for r, f in b_accepted[1:]:
        ax.scatter([r], [f], s=150, marker='D', color='#E24A4A',
                   edgecolor='black', linewidth=1.5, zorder=10)
    ...
    if len(b_accepted) > 1:
        for i, (r, f) in enumerate(b_accepted):
            ...
            ax.annotate(label_text, xy=(r, f), xytext=(5, 10),
                       textcoords='offset points', fontsize=8,
                       bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                                edgecolor='#E24A4A', alpha=0.8))

    # Styling
    ax.set_xlabel('Round Number', fontsize=12, fontweight='bold')
    ax.set_ylabel('Champion Fitness', fontsize=12, fontweight='bold')
    ax.set_title('M25 Co-Evolution: Champion Progression\nStep Function Shows Only Accepted Champions',
                 fontsize=14, fontweight='bold', pad=20)
    ax.grid(True, alpha=0.3, linestyle=':', linewidth=0.5)
    ...
    if len(b_accepted) > 1:
        for i in range(len(b_accepted) - 1):
            ...
            if abs(delta) > 0.05:  # Only show significant changes
                mid_r = (r1 + r2) / 2
                ax.annotate('', xy=(r2, f2), xytext=(r1, f1),
                           arrowprops=dict(arrowstyle='->', color='#E24A4A',
...
def plot_red_queen_effect(team_a: list, team_b: list, out_path: Path):
    ...
    # Plot area chart
    ax.fill_between(rounds, advantage, 0, where=np.array(advantage) >= 0,
                     color='#4A90E2', alpha=0.3, label='Team A Advantage',
                     interpolate=True)
    ax.fill_between(rounds, advantage, 0, where=np.array(advantage) < 0,
                     color='#E24A4A', alpha=0.3, label='Team B Advantage',
                     interpolate=True)
        ...
        for i in range(len(advantage) - 1):
            if (advantage[i] > 0 and advantage[i+1] < 0) or \
               (advantage[i] < 0 and advantage[i+1] > 0):
                ...
                ax.axvline(cross_round, color='orange', linestyle=':',
                          linewidth=2, alpha=0.7)
                ax.annotate(f'Balance Shift\n(Round {cross_round})',
                           xy=(cross_round, 0), xytext=(cross_round + 5, 0.5),
    ...
    ax.set_title('M25 Co-Evolution: Red Queen Effect\nCompetitive Balance Over Time',
                 fontsize=14, fontweight='bold', pad=20)
    ax.grid(True, alpha=0.3, linestyle=':', linewidth=0.5)
...
def main():
    ...
    print(f"  1. fig1_arms_race_timeline.png  - Dual-line fitness evolution")
    print(f"  2. fig2_champion_staircase.png  - Champion progression steps")
    print(f"  3. fig3_red_queen_effect.png    - Competitive balance shifts")
