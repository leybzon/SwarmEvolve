...
def plot_strategy_progression_tree(team_b: list, out_path: Path):
    ...
    fig.suptitle('M25 Co-Evolution: Strategy Progression Tree\nLineage of Successful Tactics (Team B)',
    ...
    for i, node in enumerate(tree_data):
        ...
        if node['fitness'] > 0.5 and (level == 0 or tree_data[level-1]['fitness'] < 0.5):
            ax.text(x_pos - box_width/2 - 0.3, y_pos, '★',
                   fontsize=24, color='gold', va='center', ha='center', zorder=5)

        # Round and fitness (left)
        ax.text(x_pos - box_width/2 + 0.2, y_pos, f'R{node["round"]}',
               fontsize=9, fontweight='bold', va='center')

        fitness_color = '#27AE60' if node['fitness'] > 0 else '#E74C3C'
        ax.text(x_pos - box_width/2 + 0.8, y_pos, f'{node["fitness"]:+.2f}',
               fontsize=9, fontweight='bold', color=fitness_color, va='center')

        # Strategy name (center)
        strategy_short = node['strategy'][:30] + '...' if len(node['strategy']) > 30 else node['strategy']
        ax.text(x_pos, y_pos, strategy_short,
               fontsize=9, va='center', ha='center', fontweight='bold')

        # Tactic type (right)
        ax.text(x_pos + box_width/2 - 0.2, y_pos, node['tactic_type'],
               fontsize=7, va='center', ha='right', style='italic', color='#555')
    ...
    ax.legend(handles=legend_elements, loc='upper left', fontsize=9,
             title='Tactic Types', framealpha=0.9)

    # Add annotations
    ax.text(0.5, 0.5, 'Lineage shows\nevolutionary path\nfrom simple to\ncomplex tactics',
...
def plot_counter_tactic_network(team_a: list, team_b: list, out_path: Path):
    ...
    for i, entry_b in enumerate(accepted_b[1:], 1):
        ...
        if fitness_delta > 0:
            G.add_edge(node_from, node_to,
                      edge_type='evolution', weight=fitness_delta,
                      label=f'+{fitness_delta:.2f}')
                    ...
                    if node_a in G and node_b in G:
                        ...
                        if entry_b['fitness'] > 0.5:
                            G.add_edge(node_b, node_a,
                                     edge_type='counter', weight=entry_b['fitness'],
                                     label='counters')
    ...
    # Draw edges
    evolution_edges = [(u, v) for u, v, d in G.edges(data=True) if d.get('edge_type') == 'evolution']
    counter_edges = [(u, v) for u, v, d in G.edges(data=True) if d.get('edge_type') == 'counter']

    nx.draw_networkx_edges(G, pos, edgelist=evolution_edges,
                          edge_color='gray', arrows=True, arrowsize=20,
                          width=2, alpha=0.5, ax=ax, connectionstyle='arc3,rad=0.1')

    nx.draw_networkx_edges(G, pos, edgelist=counter_edges,
                          edge_color='red', arrows=True, arrowsize=25,
                          width=3, alpha=0.7, ax=ax, style='dashed',
    ...
    # Title and legend
    ax.set_title('M25 Co-Evolution: Counter-Tactic Network\nDirected Graph of Strategic Relationships',
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], color='red', linewidth=3, linestyle='--', label='Counters (cross-team)'),
    ]
...
def plot_emergent_behavior_timeline(team_b: list, out_path: Path):
    ...
    ax.plot(rounds, fitness_vals, 'o-', color='blue', linewidth=2, markersize=8, label='Champion Fitness')
    ...
    ax.set_title('M25 Co-Evolution: Emergent Behavior Timeline\nQualitative Analysis of Tactical Development (Team B)',
                fontsize=14, fontweight='bold', pad=20)
    ...
    for i in range(min(len(phase_boundaries) - 1, len(phase_labels))):
        ...
        ax.text(mid, -1.3, phase_labels[i],
...
def main():
    ...
    print(f"  6. fig6_strategy_progression.png     - Hierarchical descent tree")
    print(f"  7. fig7_counter_tactic_network.png   - Network graph of who counters whom")
    print(f"  8. fig8_emergent_behaviors.png       - Qualitative behavior timeline")
