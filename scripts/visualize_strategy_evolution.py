...
def plot_strategy_timeline(team_b: list, run_dir: Path, out_path: Path):
    ...
    for i, entry in enumerate(accepted):
        ...
        ax.text(0.7, y_pos, f"R{round_num}", fontsize=11, fontweight="bold", va="center")
        ...
        ax.text(2.5, y_pos, strategy, fontsize=10, fontweight="bold", va="center", ha="left")
        ...
        if i > 0:
            prev_cat = categorize_strategy(
                accepted[i - 1]["hypothesis_tested"], accepted[i - 1].get("mechanism_expected", "")
            )
        ...
        if fitness > 0.5 and (i == 0 or accepted[i - 1]["fitness"] < 0.5):
            ax.text(0.2, y_pos, "★", fontsize=20, color="gold", va="center", ha="center")
    ...
    legend_elements = [
        mpatches.Patch(
            facecolor=tactic_colors["baseline"], edgecolor="black", label="Baseline Pursuit"
        ),
        mpatches.Patch(
            facecolor=tactic_colors["coordination"], edgecolor="black", label="Message Coordination"
        ),
        mpatches.Patch(
            facecolor=tactic_colors["kiting"], edgecolor="black", label="Kiting Tactics"
        ),
        mpatches.Patch(
            facecolor=tactic_colors["prediction"], edgecolor="black", label="Predictive Intercept"
        ),
        mpatches.Patch(
            facecolor=tactic_colors["formation"], edgecolor="black", label="Formation Control"
        ),
    ...
    for y_top, y_bottom, label, color in phases:
        if y_top > 0 and y_bottom > 0:
            ax.add_patch(
                plt.Rectangle(
                    (0.05, y_bottom), 0.4, y_top - y_bottom, facecolor=color, alpha=0.3, zorder=0
                )
            )
            ax.text(
                0.25,
...
def plot_code_complexity(team_a: list, team_b: list, run_dir: Path, out_path: Path):
    ...
    ax.set_title(
        "M25 Co-Evolution: Code Complexity vs Performance\nDoes Winning Require More Complex Code?",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def main():
    ...
    print("  4. fig4_strategy_timeline.png   - Visual flow of tactical evolution")
    print("  5. fig5_code_complexity.png     - Complexity vs performance scatter")
