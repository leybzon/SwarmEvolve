...
def create_fitness_timeline():
    ...
    team_a_fitness = np.array([1.0, 0.95, 0.9, 0.8, 0.75, 0.7, 0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1])

    # Team B fitness (rising from -0.8 to +0.9)
    team_b_fitness = np.array([-0.8, -0.7, -0.5, 0.0, 0.2, 0.5, 0.9, 0.85, 0.9, 0.85, 0.8, 0.75, 0.7, 0.8, 0.9])

    # Plot lines
    ax.plot(rounds, team_a_fitness, color=COLORS['team_a_blue'], linewidth=4,
            marker='o', markersize=10, label='Team A (Champion)', zorder=5)
    ax.plot(rounds, team_b_fitness, color=COLORS['team_b_red'], linewidth=4,
            marker='s', markersize=10, label='Team B (Challenger)', zorder=5)

    # Highlight breakthrough at Round 31
    ax.axvline(31, color=COLORS['breakthrough_green'], linestyle='--', linewidth=2, alpha=0.5)
    ax.annotate('Breakthrough\nRound 31', xy=(31, 0.9), xytext=(35, 1.0),
                fontsize=16, color=COLORS['breakthrough_green'], weight='bold',
    ...
    ax.annotate('Parity\nRound 13', xy=(13, 0.0), xytext=(15, -0.5),
                fontsize=14, color=COLORS['highlight_gold'], weight='bold',
    ...
    ax.set_title('Fitness Evolution: The Red Queen Effect', fontsize=22, weight='bold',
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'fitness_timeline.png', dpi=300, facecolor=COLORS['bg_dark'], bbox_inches='tight')
...
def create_tactical_staircase():
    ...
    for i in range(len(phases) - 1):
        ...
        # Vertical jump
        if i < len(phases) - 1:
            ax.plot([r2, r2], [f1, f2], color=COLORS['breakthrough_green'],
                   linewidth=6, linestyle='--', zorder=4)

        # Label
        ax.text(r1 + (r2 - r1) / 2, f1 - 0.15, label1, ha='center', va='top',
               fontsize=12, color=COLORS['text_primary'], weight='bold',
               bbox=dict(boxstyle='round', facecolor=COLORS['bg_dark'],
                        edgecolor=COLORS['team_b_red'], linewidth=2))

    # Final phase
    r_final, f_final, label_final = phases[-1]
    ax.plot([phases[-2][0], r_final], [f_final, f_final], color=COLORS['team_b_red'], linewidth=6, zorder=3)
    ax.text(r_final - 10, f_final - 0.15, label_final, ha='center', va='top',
           fontsize=12, color=COLORS['text_primary'], weight='bold',
           bbox=dict(boxstyle='round', facecolor=COLORS['bg_dark'],
                    edgecolor=COLORS['team_b_red'], linewidth=2))
    ...
    ax.set_xlabel('Round Number', fontsize=16, color=COLORS['text_primary'], weight='bold')
    ax.set_ylabel('Fitness', fontsize=16, color=COLORS['text_primary'], weight='bold')
    ax.set_title('Punctuated Equilibrium: Staircase Evolution', fontsize=22, weight='bold',
                 color=COLORS['text_primary'], pad=20)
    ax.grid(True, alpha=0.2, color=COLORS['text_secondary'])
    ax.set_xlim(0, 100)
    ax.set_ylim(-1.0, 1.2)
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'tactical_staircase.png', dpi=300, facecolor=COLORS['bg_dark'], bbox_inches='tight')
...
def create_code_growth():
    ...
    # Annotate key points
    ax.annotate('Baseline\n66 LOC', xy=(1, 66), xytext=(5, 100),
               fontsize=14, color=COLORS['text_primary'], weight='bold',
               arrowprops=dict(arrowstyle='->', color=COLORS['text_primary'], lw=2))

    ax.annotate('Breakthrough\n187 LOC (+183%)', xy=(31, 187), xytext=(35, 230),
               fontsize=14, color=COLORS['breakthrough_green'], weight='bold',
               arrowprops=dict(arrowstyle='->', color=COLORS['breakthrough_green'], lw=2))

    ax.set_xlabel('Round Number', fontsize=16, color=COLORS['text_primary'], weight='bold')
    ax.set_ylabel('Lines of Code', fontsize=16, color=COLORS['text_primary'], weight='bold')
    ax.set_title('Code Complexity Growth (Team B)', fontsize=22, weight='bold',
                 color=COLORS['text_primary'], pad=20)
    ax.grid(True, alpha=0.2, color=COLORS['text_secondary'])
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 250)
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'code_growth.png', dpi=300, facecolor=COLORS['bg_dark'], bbox_inches='tight')
...
def create_learning_speed_comparison():
    ...
    bars = ax.bar(categories, rounds_to_parity, color=colors, edgecolor='white', linewidth=2, width=0.6)

    for i, (bar, val) in enumerate(zip(bars, rounds_to_parity)):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width() / 2, height + 0.5,
               f'{val} rounds', ha='center', va='bottom',
               fontsize=18, weight='bold', color=colors[i])

    # Add speedup annotation
    ax.annotate('', xy=(1, 13), xytext=(0, 18),
               arrowprops=dict(arrowstyle='<->', color=COLORS['highlight_gold'], lw=3))
    ax.text(0.5, 15.5, '35% faster', ha='center', va='center',
           fontsize=16, weight='bold', color=COLORS['highlight_gold'],
           bbox=dict(boxstyle='round', facecolor=COLORS['bg_dark'],
                    edgecolor=COLORS['highlight_gold'], linewidth=2))

    ax.set_ylabel('Rounds to Reach Parity (0.0 Fitness)', fontsize=16,
                 color=COLORS['text_primary'], weight='bold')
    ax.set_title('Learning Speed: Co-Evolution Advantage', fontsize=22, weight='bold',
                 color=COLORS['text_primary'], pad=20)
    ax.set_ylim(0, 25)
    ax.grid(True, axis='y', alpha=0.2, color=COLORS['text_secondary'])

    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'learning_speed_comparison.png', dpi=300,
...
def create_team_a_stagnation():
    ...
    for bar, val in zip(bars, values):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2, height + 1,
               ...
               fontsize=24, weight='bold', color=COLORS['text_primary'])

    ax.set_ylabel('Count', fontsize=16, color=COLORS['text_primary'], weight='bold')
    ax.set_title('Team A: Trapped in Local Optimum', fontsize=22, weight='bold',
                 color=COLORS['text_primary'], pad=20)
    ax.set_ylim(0, 60)
    ax.grid(True, axis='y', alpha=0.2, color=COLORS['text_secondary'])

    # Add annotation
    ax.text(1.5, 30, 'Every mutation\nbroke critical logic', ha='center', va='center',
           fontsize=16, style='italic', color=COLORS['team_a_blue'],
           bbox=dict(boxstyle='round', facecolor=COLORS['bg_dark'],
                    edgecolor=COLORS['team_a_blue'], linewidth=2))

    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'team_a_stagnation.png', dpi=300,
               facecolor=COLORS['bg_dark'], bbox_inches='tight')
...
def create_team_b_acceptance_rate():
    ...
    ax.set_title('Team B: Adaptive Success', fontsize=22, weight='bold',
                 color=COLORS['text_primary'], pad=20)
    ax.set_ylim(0, 110)
    ax.grid(True, axis='y', alpha=0.2, color=COLORS['text_secondary'])

    # Add annotation
    ax.text(1.5, 60, 'Started simple,\nstayed flexible', ha='center', va='center',
           fontsize=16, style='italic', color=COLORS['team_b_red'],
           bbox=dict(boxstyle='round', facecolor=COLORS['bg_dark'],
                    edgecolor=COLORS['team_b_red'], linewidth=2))

    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'team_b_acceptance_rate.png', dpi=300,
               facecolor=COLORS['bg_dark'], bbox_inches='tight')
