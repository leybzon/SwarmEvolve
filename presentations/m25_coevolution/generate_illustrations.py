...
def create_phylogenetic_tree():
    ...
    _fig, ax = plt.subplots(figsize=(12, 8), facecolor=COLORS["bg_dark"])
    set_dark_style(ax)
    ...
    ax.axis("off")
    ax.set_title(
        "Code Evolution: From Baseline to Breakthrough", fontsize=18, weight="bold", pad=20
    )
...
def create_three_paradigms():
    ...
    _fig, axes = plt.subplots(1, 3, figsize=(16, 5), facecolor=COLORS["bg_dark"])
    ...
    rect = patches.Rectangle(
        ...
        alpha=0.3,
        edgecolor=COLORS["warning_magenta"],
        linewidth=2,
    )
    ax.add_patch(rect)
    ax.set_xlim(0, 1)
    ...
    # Draw petri dish
    circle = plt.Circle(
        (0.5, 0.5), 0.35, fill=False, edgecolor=COLORS["breakthrough_green"], linewidth=3
    )
    ax.add_patch(circle)
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "three_paradigms.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_arena_diagram():
    ...
    _fig, ax = plt.subplots(figsize=(10, 10), facecolor=COLORS["bg_dark"])
    set_dark_style(ax)
    ...
    for x, y in zip(team_a_x[:3], team_a_y[:3], strict=False):
        circle = plt.Circle(
            (x, y),
            100,
            fill=False,
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "arena_diagram.png", dpi=300, facecolor=COLORS["bg_dark"], bbox_inches="tight"
    )
...
def create_tactical_timeline():
    ...
    _fig, ax = plt.subplots(figsize=(14, 6), facecolor=COLORS["bg_dark"])
    set_dark_style(ax)
    ...
    colors = [
        ...
        COLORS["breakthrough_green"],
        COLORS["team_b_red"],
        COLORS["team_b_red"],
    ]

    # Bars
    for i, (r, f, t, c) in enumerate(zip(rounds, fitness, tactics, colors, strict=False)):
    ...
    ax.set_title(
        "Tactical Evolution: Punctuated Equilibrium in Action", fontsize=16, weight="bold", pad=20
    )
    ax.grid(axis="y", alpha=0.2)

    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "tactical_timeline.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_loc_fitness_scatter():
    ...
    _fig, ax = plt.subplots(figsize=(10, 8), facecolor=COLORS["bg_dark"])
    set_dark_style(ax)
    ...
    ax.scatter(
        ...
        s=100,
        alpha=0.4,
        label="Team A Failed Mutations (n=47)",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "loc_fitness_scatter.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_predator_prey_graph():
    ...
    _fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(14, 10), facecolor=COLORS["bg_dark"], sharex=True
    )
    ...
    ax1.set_title(
        "Classic Predator-Prey Dynamics (Lotka-Volterra)", fontsize=16, weight="bold", pad=15
    )
    ...
    set_dark_style(ax2)
    ax2.plot(
        rounds, team_a_fit, color=COLORS["team_a_blue"], linewidth=3, label="Team A (Champion)"
    )
    ax2.plot(
        rounds, team_b_fit, color=COLORS["team_b_red"], linewidth=3, label="Team B (Challenger)"
    )
    ax2.axhline(0, color=COLORS["text_secondary"], linestyle="--", linewidth=1, alpha=0.5)
    # Crossover point
    crossover_round = 31
    ax2.axvline(
        crossover_round, color=COLORS["breakthrough_green"], linestyle=":", linewidth=2, alpha=0.7
    )
    ax2.text(
        crossover_round + 2,
        0.5,
    ...
    ax2.set_title("Co-Evolutionary Arms Race: Red Queen Effect", fontsize=16, weight="bold", pad=15)
    ax2.legend(fontsize=12, loc="upper left")
    ax2.grid(alpha=0.2)

    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "predator_prey_graph.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_future_applications():
    ...
    _fig, axes = plt.subplots(2, 2, figsize=(14, 12), facecolor=COLORS["bg_dark"])
    ...
    for ax, (title, emoji, color) in zip(axes.flat, apps, strict=False):
        set_dark_style(ax)
        ax.text(0.5, 0.6, emoji, fontsize=100, ha="center", va="center")
        ax.text(0.5, 0.2, title, fontsize=18, ha="center", va="center", color=color, weight="bold")
        # Border
        rect = patches.Rectangle((0.05, 0.05), 0.9, 0.9, fill=False, edgecolor=color, linewidth=3)
        ax.add_patch(rect)
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.axis("off")
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "future_applications.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_evolution_vs_engineering():
    ...
    _fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8), facecolor=COLORS["bg_dark"])
    ...
    for i, (x, y, label) in enumerate(beak_types):
        ...
        ax1.plot([x - beak_length, x], [y, y], color=COLORS["highlight_gold"], linewidth=10 - i * 2)
        # Head circle
        circle = plt.Circle((x - beak_length, y), 0.05, color=COLORS["text_secondary"])
        ax1.add_patch(circle)
        ax1.text(
            x - beak_length, y - 0.12, label, ha="center", fontsize=11, color=COLORS["text_primary"]
        )
    ...
    for x, y in [(0.2, 0.7), (0.4, 0.5), (0.6, 0.3), (0.8, 0.3)]:
        rect = patches.Rectangle(
            (x - 0.04, y - 0.04), 0.08, 0.08, facecolor=COLORS["team_a_blue"], alpha=0.6
        )
        ax2.add_patch(rect)
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "evolution_vs_engineering.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
