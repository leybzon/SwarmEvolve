...
def fix_slide_overflow():
    ...
    html = re.sub(
        r'<div class="tactic-grid">', r'<div class="tactic-grid" style="margin-top: 1rem;">', html
    )

    # Slide 5: Make tactic cards more compact
    html = re.sub(r"(\.tactic-card \{[^}]*?)padding: 1\.5rem;", r"\1padding: 1rem;", html)

    # Slide 6: Reduce code panel sizes
    html = re.sub(r"(\.code-split \{[^}]*?)gap: 1\.5rem;", r"\1gap: 1rem;", html)
    ...
    html = re.sub(r"(\.reveal h2 \{[^}]*?)margin-bottom: 1\.5rem;", r"\1margin-bottom: 1rem;", html)

    # Global: Reduce h3 bottom margin
    html = re.sub(r"(\.reveal h3 \{[^}]*?)margin-bottom: 2rem;", r"\1margin-bottom: 1rem;", html)
    ...
    html = re.sub(
        r"(\.diagram-container img[^}]*?\{[^}]*?)", r"\1\n            max-height: 450px;", html
    )

    # Reduce reveal.js default margins
    html = re.sub(r"margin: 0\.04,", r"margin: 0.02,", html)
