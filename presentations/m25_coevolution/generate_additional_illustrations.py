...
def create_darwins_finches():
    ...
    for _i, finch in enumerate(finches):
        x_center = 0.3
        y = finch["y"]
    ...
    ax.text(
        0.5,
        0.95,
        "Darwin's Finches: Beak Adaptation",
        ha="center",
        fontsize=18,
        weight="bold",
        color=COLORS["text_primary"],
    ...
    ax.text(
        0.5,
        0.05,
        "Natural Selection: Form Follows Function",
        ha="center",
        fontsize=14,
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "darwins_finches.png",
        dpi=300,
...
def create_code_phylogeny():
    ...
    _fig, ax = plt.subplots(figsize=(12, 10), facecolor=COLORS["bg_dark"])
    set_dark_style(ax)
    ...
    connections = [(0, 1), (0, 2), (2, 3), (2, 4), (3, 5), (4, 5)]
    ...
    for x, y, label, code, color, is_breakthrough in tree_data:
        ...
        ax.scatter(
            x, y, s=markersize, c=color, marker=marker, edgecolors="white", linewidths=2, zorder=10
        )
    ...
    ax.axis("off")
    ax.set_title(
        "Code Evolution: Phylogenetic Tree\nFrom pursuit_v1 to Zone Control",
        fontsize=18,
        weight="bold",
        pad=20,
        color=COLORS["text_primary"],
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "code_phylogeny.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
...
def create_scientist_portraits():
    ...
    _fig, axes = plt.subplots(1, 5, figsize=(18, 4), facecolor=COLORS["bg_dark"])
    ...
    for ax, (name, years, contribution, color) in zip(axes, scientists, strict=False):
        set_dark_style(ax)

        # Portrait silhouette (circle with initials)
        circle = plt.Circle(
            (0.5, 0.6), 0.25, facecolor=color, alpha=0.3, edgecolor=color, linewidth=3
        )
        ax.add_patch(circle)
        ...
        ax.text(
            0.5, 0.6, initials, ha="center", va="center", fontsize=36, weight="bold", color=color
        )
        ...
        # Years
        ax.text(
            0.5, 0.15, years, ha="center", va="center", fontsize=10, color=COLORS["text_secondary"]
        )
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "scientist_portraits.png",
        dpi=300,
        facecolor=COLORS["bg_dark"],
        bbox_inches="tight",
    )
